...
- **Two-GitHub-account support is work-machine-only, but safe everywhere.** On machines with both a work and personal GitHub account, `.zshrc` defines a `gh` wrapper + `ghsync` that align the active `gh` account to the repo (`github.com-personal` remote → personal, plain `github.com` → work), and `ssh/config.example` documents the matching SSH host-alias + connection-multiplexing fix. Account names are hardcoded in `_gh_account_for_repo`. On a personal-only machine the wrapper is a no-op (no `github.com-personal` remotes to match), so it's harmless to keep. `~/.ssh/config` itself is NOT tracked (machine/work-specific, sits next to a private key) — only the sanitized example is.
